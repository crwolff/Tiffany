...
class ColorQToolButton(QToolButton):
    ...
    def setIcon(self):
        ...
        painter.fillRect(1,1,34,34,QtCore.Qt.red)
        painter.fillRect(29,29,62,62,QtCore.Qt.blue)
        painter.end()
    ...
    def mouseMoveEvent(self, event):
        if (event.pos().x() <= 25) and (event.pos().y() <= 15):
            ...
            self.setToolTip("Swap Foreground/Background")
            self.mode = "Swap"
        elif (event.pos().x() <= 25):
            self.setToolTip("Reset Colors")
            self.mode = "Reset"
        elif (event.pos().x() > 25):
